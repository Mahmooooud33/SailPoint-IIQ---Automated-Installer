...
# Add this new step after step 5.1 (Add SQL Server JDBC Driver) and before step 6 (Create IIQ Database Schema)
...
# ------------------------------
Write-Output ">>> Modifying IdentityExtended.hbm.xml to uncomment extended properties..."

$IdentityExtendedHbmPath = "$TomcatPath\webapps\identityiq\WEB-INF\classes\sailpoint\object\IdentityExtended.hbm.xml"

if (!(Test-Path $IdentityExtendedHbmPath)) {
    Write-Error "IdentityExtended.hbm.xml not found at: $IdentityExtendedHbmPath"
    exit 1
}

# Read the file as string content
$fileContent = Get-Content $IdentityExtendedHbmPath -Raw
$originalContent = $fileContent

# Check if extended properties 11-20 are already uncommented
$extendedPattern = 'property name="extended1[1-9]"'
$extended20Pattern = 'property name="extended20"'
$commentPattern = '<!--'

$hasExtended11to19 = $fileContent -match $extendedPattern
$hasExtended20 = $fileContent -match $extended20Pattern
$hasComments = $fileContent -match $commentPattern

Write-Output ">>> File analysis:"
Write-Output "    Has extended11-19 properties: $hasExtended11to19"
Write-Output "    Has extended20 property: $hasExtended20"
Write-Output "    Has XML comments: $hasComments"

# Simple approach: look for and uncomment the specific block
# Look for the pattern more specifically
if ($fileContent -like "*<!--*extended11*extended20*-->*") {
    Write-Output ">>> Found commented extended properties block. Uncommenting..."
    
    # Define the exact replacement - what we want the final result to look like
    $propertyBlock = @"
...
"@
    ...
    # Find and replace the commented section using a more precise method
    $lines = Get-Content $IdentityExtendedHbmPath
    $newLines = @()
    $inCommentBlock = $false
    $foundStartOfExtended = $false
    $blockReplaced = $false
    
    for ($i = 0; $i -lt $lines.Count; $i++) {
        $line = $lines[$i]
        
        # Look for start of comment that contains extended11
        if ($line -like "*<!--*" -and !$inCommentBlock) {
            # Check if this comment block contains extended properties
            $lookAhead = ""
            for ($j = $i; $j -lt [Math]::Min($i + 15, $lines.Count); $j++) {
                $lookAhead += $lines[$j]
            }
            
            if ($lookAhead -like "*extended11*" -and $lookAhead -like "*extended20*") {
                $inCommentBlock = $true
                $foundStartOfExtended = $true
                # Add the properly formatted properties instead
                $propertyBlock -split "`r`n|`n" | ForEach-Object { 
                    if ($_.Trim()) { $newLines += $_ }
                }
                $blockReplaced = $true
                continue
            }
        ...
        # Skip lines while we're in the comment block
        if ($inCommentBlock) {
            if ($line -like "*-->*") {
                $inCommentBlock = $false
            }
    ...
    if ($blockReplaced) {
        ...
        Write-Output ">>> IdentityExtended.hbm.xml successfully modified - extended properties 11-20 uncommented with proper formatting."
    } else {
        Write-Output ">>> No commented extended properties block was replaced."
    }
} else {
    # Alternative: try line-by-line processing
    $lines = Get-Content $IdentityExtendedHbmPath
    $newLines = @()
    $modified = $false
    ...
    foreach ($line in $lines) {
        ...
        # Check if this line contains a commented extended property 11-20
        if ($line -like "*extended1*" -or $line -like "*extended20*") {
            if ($line -like "*<!--*" -and $line -like "*-->*") {
                # Single line comment - remove comment markers
                $newLine = $line -replace '<!--\s*', '' -replace '\s*-->', ''
                $newLine = "    " + $newLine.Trim()
                $modified = $true
                Write-Output ">>> Uncommented: $newLine"
            }
            elseif ($line -like "*<!--*") {
    ...
    if ($modified) {
        ...
        $Utf8NoBomEncoding = New-Object System.Text.UTF8Encoding($false)
        [System.IO.File]::WriteAllText($IdentityExtendedHbmPath, $newContent, $Utf8NoBomEncoding)
        Write-Output ">>> IdentityExtended.hbm.xml successfully modified using line-by-line processing."
    } else {
        # Check if properties already exist uncommented
        $uncommentedProps = $lines | Where-Object { $_ -like "*property name=`"extended1*" -and $_ -notlike "*<!--*" }
        
        if ($uncommentedProps.Count -gt 0) {
            Write-Output ">>> Extended properties appear to already be uncommented:"
            $uncommentedProps | ForEach-Object { Write-Output "    $_" }
        } else {
            Write-Warning ">>> No commented or uncommented extended properties 11-20 found in the file."
            Write-Output ">>> Showing all property definitions found:"
            $allProps = $lines | Where-Object { $_ -like "*property name=*" }
            if ($allProps.Count -gt 0) {
                $allProps | ForEach-Object { Write-Output "    $_" }
            } else {
                Write-Output "    No property definitions found in the file."
            }
        }
    }
}
